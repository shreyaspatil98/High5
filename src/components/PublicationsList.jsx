...
export default function PublicationsList() {
  ...
  const [visualizePub, setVisualizePub] = useState(null);
  ...
  if (loading) return <div className="text-center py-8 text-white">Loading publications...</div>;

  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-8 text-white">Space Biology Publications ({publications.length})</h1>
      ...
      {selectedPub ? (
        <>
          <button onClick={() => { setSelectedPub(null); setVisualizePub(null); }} className="mb-4 px-4 py-2 bg-gray-200 rounded">
            ← Back to list
          </button>
          ...
          <button
            onClick={() => setVisualizePub(selectedPub)}
            className="mt-4 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
          >
            Visualize 📊
          </button>

          {visualizePub && <GraphicalAnalysis publication={visualizePub} />}
        </>
      ) : (
        <div className="space-y-4">
          {publications.map((pub, index) => (
            <div
              ...
              onClick={() => { setSelectedPub(pub); setVisualizePub(null); }}
            >
